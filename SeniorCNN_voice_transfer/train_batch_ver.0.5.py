...
# 191103_1st Style: VC, 2nd Style: DIFF_VC
...
Style_DIFF = basepath + 'Style_DIFF/'
Respath = "output/"
...
# fStyle_DIFF
fStyle_DIFF = open(Style_DIFF+'style_DIFFList.txt', 'w', encoding="UTF-8")

for root, dirs, files in os.walk(Style_DIFF):   # 리스트 파일로 생성할 파일들이 있는 폴더 지정
    files.sort()
...
listCon = open(Contentpath+'conList.txt', 'r', encoding="UTF-8")
listSty = open(Stylepath+'styleList.txt', 'r', encoding="UTF-8")
listSty_DIFF = open(Style_DIFF+'style_DIFFList.txt', 'r', encoding="UTF-8")
...
line_Sty = listSty.readline().strip()
line_Res = listRes.readline().strip()
line_Sty_DIFF = listSty_DIFF.readline().strip()
